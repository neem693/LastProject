...
<body>
	...
	<c:forEach var="vo" items="${list}">
		...
		<%-- 선택된 파티명은 좀 다르게 표기를 한다. --%>
		<c:choose>
			<c:when test="${vo.pt_idx eq selt_pt_idx }">
				<c:set var="party_name">▶<span
						style="text-decoration: underline;">${vo.pt_name}</span>
				</c:set>
			</c:when>
			<c:otherwise>
				<c:set var="party_name">
				${vo.pt_name}
				</c:set>
			</c:otherwise>
		...
		<div class="party_template"
			...
			<div onclick="show_view(${vo.pt_idx})" class="party_name"
				style="${team_color_css}">${party_name }</div>
			<div class="team_img">
